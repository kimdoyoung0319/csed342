...
def test_eucliean_distance():
    ...
    data2 = [-5, -3, -1, -1]
    assert np.allclose(
        np.array(euclidean_distance(data1, data2)),
        np.linalg.norm(np.array(data1) - np.array(data2)).tolist(),
    )
    ...
    assert np.allclose(
        np.array(euclidean_distance(data1.tolist(), data2.tolist())),
...
def test_mean_of_points():
    ...
    assert np.allclose(np.array(mean_of_points(data.tolist())), data.mean(axis=0))
    ...
    assert np.allclose(np.array(mean_of_points(data)), np.array(data).mean(axis=0))
...
def test_update_centroids():
    ...
    answer = {"centroid1": [-0.82978110, 0.08329567, 0.04970701, -0.146715632]}
    rtn = update_centroids(assignment_dict)
    assert_dict_eq(rtn, answer)
